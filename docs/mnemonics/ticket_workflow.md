...
| **Finalising** | Leo's near-final polishing (a working state) | **Leo** |
...
- **Drafting (Open → AI Proposed):** propose birth/death pegs (from the candidates or a
  ...
  scene tied to the figure's real life, and a derived Position. **Put the chosen peg words
  in bold in the Scene** so the number-carriers stand out. Set `AI Proposed`. Stop.
...
- `scripts/youtrack_seed.py` — create/refresh tickets from `figures.yaml`.
  ...
  - `--update` refreshes summary/description only and **skips tickets in a working state**
    (AI Proposed/In Progress/Finalising/Submitted/Reopened) so drafts aren't clobbered; `--force` overrides.
